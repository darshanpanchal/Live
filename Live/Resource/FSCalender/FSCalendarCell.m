...
- (void)layoutSubviews
{
    ...
    _subtitleLabel.frame = CGRectMake(
                                      self.preferredSubtitleOffset.x+self.contentView.fs_width/4,
                                      subtitleHeight/3+1.5,//(_titleLabel.fs_bottom-self.preferredTitleOffset.y) - (_titleLabel.fs_height-_titleLabel.font.pointSize)+self.preferredSubtitleOffset.y,
    ...
    */
    ...
    _eventIndicator.frame = CGRectMake(
                                       ...
                                       25,//CGRectGetMaxY(_shapeLayer.frame)+eventSize*0.17+self.preferredEventOffset.y,
                                       self.fs_width,
                                       eventSize*0.83
                                       );
}
    ...
    if (_numberOfEvents > 0){
        ...
        if (self.placeholder){
            ...
            if (self.selected) {
                ...
                _subtitleLabel.backgroundColor = [[UIColor alloc] initWithRed:54.0/255.0 green:82.0/255.0 blue:125.0/255.0 alpha:0.6];
            }
...
{
    if (self.selected) {
        return [UIColor clearColor];//_preferredBorderSelectionColor ?: _appearance.borderSelectionColor;
    }
    if (_numberOfEvents > 0){
        return [[UIColor alloc] initWithRed:175.0/255.0 green:209.0/255.0 blue:53.0/255.0 alpha:1.0];//_preferredBorderDefaultColor ?: _appearance.borderDefaultColor;
    }else{
        return [UIColor clearColor];
    }
